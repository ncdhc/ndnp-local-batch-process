...
REM if the batch spreadsheet file exists as entered, proceed
if exist "%~dp0data\%XML%.xml" (

REM use the saxon transform executable to regenerate issue-level METS files
"%~dp0lib\saxon\Transform" -s:"%~dp0files\%XML%.xml" -xsl:"%~dp0lib\makeMETS.xsl"
echo METS files regenerated!

REM use the saxon transform executable to regenerate batch-level catalog file
